...
const parseBenchmarkMetadata = (rawMetadata: Record<string, unknown>) => {
  ...
  let systemMetadata: BenchmarkSystemMetadata = {
  ...
  if (system_metadata.cpu_count) {
    systemMetadata.cpuCount = system_metadata.cpu_count as number;
  }
  if (system_metadata.cpu_model) {
    systemMetadata.cpuModel = system_metadata.cpu_model as string;
  }
  if (system_metadata.cpu_freq) {
    systemMetadata.cpuFreq = system_metadata.cpu_freq as string;
  }
